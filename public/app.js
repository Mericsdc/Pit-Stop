...
function channelOptions(selected, voice = false) {
  return `<option value="">Kanal seçin</option>` + state.guild.channels.filter(channel => voice ? channel.type === 2 : [0, 5].includes(channel.type)).map(channel => `<option value="${escape(channel.id)}" ${selected === channel.id ? 'selected' : ''}>${voice ? '♫' : '#'} ${escape(channel.name)}</option>`).join('');
}
