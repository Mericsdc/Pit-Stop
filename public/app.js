...
function crewBody(crew) {
  ...
  return `<div class="stats"><div class="card stat"><div class="stat-label">Ekibin toplam REP'i</div><div class="stat-value">${number(crew.crewRep)}</div><div class="stat-foot">Güncel toplam</div></div><div class="card stat"><div class="stat-label">Anlık REP</div><div class="stat-value">${signed(crew.instantCrewRep)}</div><div class="stat-foot">Son yenilemeden beri</div></div><div class="card stat"><div class="stat-label">Bugünkü REP</div><div class="stat-value">${signed(crew.dailyCrewRep)}</div><div class="stat-foot">Günün referans kaydına göre</div></div><div class="card stat"><div class="stat-label">Bugünkü etkinlik</div><div class="stat-value">${signed(crew.dailyEvents)}</div><div class="stat-foot">Tamamlanan etkinlik farkı</div></div><div class="card stat"><div class="stat-label">Takip edilen üye</div><div class="stat-value">${number(crew.members.length)}</div><div class="stat-foot">Ekip kadrosu</div></div></div>${crew.error || crew.rosterError ? `<div class="notice error">${escape(crew.error || crew.rosterError)}</div>` : ''}${!crew.exactRoster ? '<div class="notice error">NightRiderz Üyeler bağlantısı bekleniyor. Anlık ve günlük REP değerleri, canlı üye REP erişimi yapılandırıldığında otomatik hesaplanır.</div>' : ''}<section class="card"><div class="card-header"><div><h3>Üye karşılaştırması</h3><p class="muted tiny">Son yenileme: ${date(crew.updatedAt)} · ${comparisonText}</p></div><a class="button subtle" href="${escape(crew.sourceUrl)}" target="_blank" rel="noopener noreferrer">Kaynağı aç ↗</a></div><div class="crew-legend"><span class="trend up">↑ Yeşil: REP kazandı</span><span class="trend down">↓ Kırmızı: REP kazanmadı veya azaldı</span><span class="trend neutral">• Anlık: son yenilemeye göre</span></div><div class="table-wrap"><table><thead><tr><th>ÜYE</th><th>EKİP REP'İ</th><th>ANLIK REP</th><th><button type="button" class="table-sort" data-crew-sort aria-label="Bugünkü REP değerine göre ${direction === 'asc' ? 'büyükten küçüğe' : 'küçükten büyüğe'} sırala">BUGÜNKÜ REP <span aria-hidden="true">${sortArrow}</span></button></th><th>SON GİRİŞ</th><th>TAMAMLANAN ETKİNLİKLER</th><th>BUGÜNKÜ ETKİNLİK</th><th>SÜRÜCÜ PUANI</th><th>BUGÜNKÜ PUAN</th></tr></thead><tbody>${members.map(member => `<tr><td><strong>${escape(member.name)}</strong><small class="muted">Seviye ${number(member.level)}</small></td><td>${number(member.crewRep)}</td><td>${instantTrend(member)}</td><td>${crewTrend(member, crew.comparisonAvailable)}</td><td class="time">${escape(member.lastLogin || '—')}</td><td>${number(member.eventsCompleted)}</td><td>${signed(member.dailyEvents)}</td><td>${number(member.driverScore)}</td><td>${signed(member.dailyDriverScore)}</td></tr>`).join('')}</tbody></table></div><p class="muted tiny crew-source">${crew.exactRoster ? 'Üye listesi ve REP değerleri NightRiderz Üyeler bölümünden canlı alındı.' : 'Üye listesi son doğrulanan ekip kadrosundan, profil alanları NightRiderz API üzerinden canlı alındı.'}${crew.profileFailures ? ` ${number(crew.profileFailures)} profil son başarılı değeri korudu.` : ''}</p></section>`;
}
...
setInterval(async () => {
  ...
  try {
    ...
    if (state.view === 'crew') await loadCrew();
