...
Crew REP paneli üç saatte bir yenilenir ve panelden elle de güncellenebilir. Otomatik yenilemeler olay kayıtlarını doldurmaz; yalnızca panelde bir yetkilinin başlattığı manuel yenileme kullanıcı adıyla kaydedilir. Profil istatistikleri NightRiderz herkese açık API’sinden alınır. `NRZ_USER_KEY` ve `NRZ_PERSONA_KEY` salt-okunur oturum değerleri ayarlanırsa üye kadrosu ve REP değerleri Members bölümünden canlı alınır; boş bırakılırsa son doğrulanan kadro ve profil ölçümleri gösterilir, canlı REP değişimi gösterilmez. Bu iki değer özel yapılandırmadır ve GitHub’a eklenmez.
